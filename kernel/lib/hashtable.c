...
struct hashtable* hashtable_create(size_t head_count, size_t key_size, size_t value_size) {
    ...
    table->heads = kzalloc(head_count * sizeof(struct hashtable_node), GFP_VM_KERNEL | GFP_PM_ZONE_NORMAL);
    if (!table->heads) {
        kfree(table);
        return NULL;
    }
...
hashtable_node_create(const void* key, const void* value, size_t key_size, size_t value_size) {
    struct hashtable_node* node = kmalloc(sizeof(struct hashtable_node), 
            GFP_VM_KERNEL | GFP_PM_ZONE_NORMAL);
    if (!node)
        return NULL;

    node->key = kmalloc(key_size, GFP_VM_KERNEL | GFP_PM_ZONE_NORMAL);
    if (!node->key) {
        kfree(node);
        return NULL;
    }
    node->value = kmalloc(value_size, GFP_VM_KERNEL | GFP_PM_ZONE_NORMAL);
    if (!node->value) {
        kfree(node->key);
        kfree(node);
        return NULL;
